...
#define SIZEOFCHAR (1)
#if 0
#if (sizeof(int) != SIZEOFINT)
#error "sizeof int"
#endif
#if (SIZEOFSHORT != sizeof(short))
#error "sizeof short"
#endif
...
_fetch_present_idx(const void *struct_ptr, int pres_offset, int pres_size) {
	...
	switch(pres_size) {
	case SIZEOFINT:	present =   *(const int *)present_ptr; break;
	case SIZEOFSHORT:	present = *(const short *)present_ptr; break;
	case SIZEOFCHAR:	present =  *(const char *)present_ptr; break;
	default:
...
_set_present_idx(void *struct_ptr, int pres_offset, int pres_size, int present) {
	...
	switch(pres_size) {
	case SIZEOFINT:	*(int *)present_ptr   = present; break;
	case SIZEOFSHORT:	*(short *)present_ptr = present; break;
	case SIZEOFCHAR:	*(char *)present_ptr  = present; break;
	default:
		/* ANSI C mandates enum to be equivalent to integer */
		assert(pres_size != sizeof(int));
	}
