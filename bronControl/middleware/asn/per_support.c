...
per_get_few_bits(asn_per_data_t *pd, int nbits) {
	 ...
	 */
	if(off <= 8)
		accum = nbits ? ((uint32_t)buf[0]) >> (8 - off) : 0;
	else if(off <= 16)
		accum = (((uint32_t)buf[0] << 8) + (uint32_t)buf[1]) >> (16 - off);
	else if(off <= 24)
		accum = (((uint32_t)buf[0] << 16) + ((uint32_t)buf[1] << 8) + (uint32_t)buf[2]) >> (24 - off);
	else if(off <= 31)
		accum = (((uint32_t)buf[0] << 24) + ((uint32_t)buf[1] << 16)
			+ ((uint32_t)buf[2] << 8) + ((uint32_t)buf[3])) >> (32 - off);
	else if(nbits <= 31) {
...
per_put_many_bits(asn_per_outp_t *po, const uint8_t *src, int nbits) {
	...
	while(nbits) {
		...
		if(nbits >= 24) {
			value = ((uint32_t)src[0] << 16) | ((uint32_t)src[1] << 8) | (uint32_t)src[2];
			src += 3;
			nbits -= 24;
